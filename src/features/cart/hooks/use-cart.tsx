...
interface ICartContext {
    ...
    addToCart: (payload: { id: string; name: string; price: number; selectedColor: string; selectedSize: string; selectedImage: string; quantity?: number }) => void;
    removeFromCart: (productId: string, options?: { color?: string; size?: string }) => void;
    incrementCartItemQuantity: (productId: string, options: { color: string; size: string }, step?: number) => void;
    decrementCartItemQuantity: (productId: string, options: { color: string; size: string }, step?: number) => void;
    setCartItemQuantity: (productId: string, options: { color: string; size: string }, quantity: number) => void;
    clearCart: () => void;
}
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
    ...
    const setCartItemQuantity = useCallback((productId: string, options: { color: string; size: string }, quantity: number) => {
    ...
    const removeFromCart = useCallback((productId: string, options?: { color?: string; size?: string }) => {
    ...
    const totalQuantity = useMemo(() => cartItems.reduce((sum, it) => sum + it.quantity, 0), [cartItems]);
    const subtotalPrice = useMemo(() => cartItems.reduce((sum, it) => sum + (it.price * it.quantity), 0), [cartItems]);
    ...
    const value = useMemo<ICartContext>(() => ({
        ...
        setCartItemQuantity,
        clearCart,
    }), [cartItems, totalQuantity, subtotalPrice, addToCart, removeFromCart, incrementCartItemQuantity, decrementCartItemQuantity, setCartItemQuantity, clearCart]);
